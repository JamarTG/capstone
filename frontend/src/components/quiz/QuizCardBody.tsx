...
const QuizCardBody: FC<Props> = ({
  ...
  return (
    <section
      className={`relative flex-1 p-4 flex flex-col justify-between ${isDark ? "text-gray-100" : "text-slate-800"}`}
    >
      {completed ? (
      ...
      <h3
        className={`text-md break-all hyphens-auto ${isDark ? "text-gray-100" : "text-slate-600"}`}
      >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-2 mt-4">
        ...
        <button
          ...
          onClick={() =>
            navigate(`/${completed ? "review" : "quiz"}/${quizId}`)
          }
        >
